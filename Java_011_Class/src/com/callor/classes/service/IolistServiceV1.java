 ...
 * 데이터의 묶음을 처리할 때는 보통 배열을 많이 사용한다
...
public class IolistServiceV1 {
	 ...
	 * 매입매출 데이터들의 묶ㅁ을 List 형 객체 사용하겠다
	 * 
	...
	while(true) {
		...
		if(iprice < 0) {
			System.out.println("입력 범위를 벗어났습니다 재입력 하세요");
			continue;
		}
		...
		iolistVOA.setPname(pname);
		iolistVOA.setDate(date);
		iolistVOA.setDname(dname);
		iolistVOA.setInout(inout);
		iolistVOA.setIprice(iprice);
		iolistVOA.setOprice(oprice);
		iolistVOA.setQty(qty);
		
		// iolistVOA에 저장한 데이터를 확인하기!!
		this.printIolist(iolistVOA);
		
		// 입력된 매입매출 데이터를 데이터리스트에 담기
		iolist.add(iolistVOA);
	}
	...
	public void printAllList() {
		...
		System.out.println("-".repeat(50));
		System.out.println(
				"상품명\t거래일\t\t거래처\t구분\t수량\t매입단가\t매출단가");
		
		for(int i = 0 ; i < iolist.size() ; i++) {
			
			// IolistVO vo = iolist[i]
			//IolistVOA vo = iolist.get(i);
			//this.printIolist(vo);   2줄의 코드를 한줄로 쓸 수 있다.
